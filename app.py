...
def show_home_section():
    ...
    # Try multiple locations for sample data file (GitHub deployment compatibility)
    possible_paths = [
        os.path.join(os.path.dirname(__file__), "data", "sample_data_10k.csv"),  # Local development
        os.path.join(os.path.dirname(__file__), "sample_data_10k.csv"),          # GitHub/root directory
        "sample_data_10k.csv"                                                     # Direct path fallback
    ]
    ...
    if sample_path:
        try:
            with open(sample_path, "rb") as f:
                st.download_button(
                    label="📄 Download sample_data_10k.csv",
                    data=f,
                    file_name="sample_data_10k.csv",
                    mime="text/csv",
                    help="Download sample healthcare data to test the dashboard features"
                )
            st.success(f"✅ Sample data found at: {os.path.basename(sample_path)}")
        except Exception as e:
            st.error(f"Error loading sample file: {e}")
    else:
        st.warning("⚠️ Sample data file not found. Please ensure 'sample_data_10k.csv' is in your project directory.")
        st.info("""
        **For GitHub deployment:** Place the sample_data_10k.csv file in the root directory of your repository.
        **For local development:** You can use either the root directory or a 'data' folder.
        """)
...
def show_demographics_section(df):
    ...
    # Age Distribution with Enhanced Visualizations
    if 'Age' in df.columns:
        st.subheader("👥 Age Distribution")
        
        viz_option = st.selectbox(
            "Choose Visualization Style:",
            ["Standard (Plotly)", "Interactive Statistical (Altair)", "Advanced Dashboard (Bokeh)"],
            key="age_viz_option"
        )
        
        if viz_option == "Interactive Statistical (Altair)" and ENHANCED_VIZ_AVAILABLE:
            # Try enhanced Altair visualization
            altair_chart = create_enhanced_visualization(viz_df, "disease_demographics")
            if altair_chart:
        ...
        # Clinical context
        st.info("""
        **Clinical Context:** Age distribution helps identify target populations for:
    ...
    if 'Gender' in df.columns:
        ...
        gender_counts = viz_df['Gender'].value_counts()
...
def show_adherence_section(df):
    ...
    if 'MPR' in df.columns:
        ...
        mpr_df['Total_Days_Supplied'] = df['Total_Days_Supplied'] if 'Total_Days_Supplied' in df.columns else np.nan
        mpr_df['Prescription_Count'] = df['Total_Prescriptions'] if 'Total_Prescriptions' in df.columns else np.nan
    else:
    ...
    # MPR Distribution with Enhanced Visualizations
    st.subheader("📊 MPR Distribution")
    
    viz_option = st.selectbox(
        "Choose MPR Visualization Style:",
        ["Standard (Plotly)", "Interactive (Altair)", "Statistical Analysis (Seaborn)"],
        key="mpr_viz_option"
    )
    
    if viz_option == "Interactive (Altair)" and ENHANCED_VIZ_AVAILABLE:
        # Try enhanced Altair visualization
        altair_chart = create_enhanced_visualization(mpr_df, "mpr_distribution", title="Interactive MPR Distribution")
        if altair_chart:
            st.altair_chart(altair_chart, use_container_width=True)
        else:
            # Fallback to standard Plotly
            fig = px.histogram(mpr_df, x='MPR', nbins=20, title="MPR Distribution (Plotly Fallback)")
            fig.add_vline(x=80, line_dash="dash", line_color="red", annotation_text="80% Threshold")
            fig.update_layout(height=400)
            st.plotly_chart(fig, use_container_width=True)
    elif viz_option == "Statistical Analysis (Seaborn)" and ENHANCED_VIZ_AVAILABLE:
        # Display Seaborn statistical plots
        seaborn_plots = create_enhanced_visualization(df, "statistical_plots")
        if seaborn_plots and 'mpr_by_disease' in seaborn_plots:
            st.pyplot(seaborn_plots['mpr_by_disease'])
        else:
            st.info("Seaborn statistical analysis requires disease type information")
            # Fallback to standard
            fig = px.histogram(mpr_df, x='MPR', nbins=20, title="MPR Distribution")
        ...
        fig = px.histogram(
            ...
            x='MPR', 
            nbins=20,
            title="Medication Possession Ratio Distribution",
            labels={'x': 'MPR (%)', 'y': 'Number of Patients'}
        )
        
        # Add adherence threshold line
        fig.add_vline(x=80, line_dash="dash", line_color="red", 
                      annotation_text="80% Adherence Threshold")
...
def show_recency_section(df):
    ...
    if 'Last_Visit_Date' in df.columns and not df['Last_Visit_Date'].isna().all():
        st.subheader("📊 Days Since Last Visit Distribution")
        ...
        if not valid_visits.empty:
            fig = px.histogram(
                ...
                x='Days_Since_Visit',
                nbins=30,
                title="Distribution of Days Since Last Visit",
                labels={'x': 'Days Since Last Visit', 'y': 'Number of Patients'}
            )
            
            # Add overdue threshold line
            fig.add_vline(x=180, line_dash="dash", line_color="red", 
                          annotation_text="180-Day Threshold")

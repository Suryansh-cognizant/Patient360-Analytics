...
def create_sidebar():
    ...
    if ENHANCED_VIZ_AVAILABLE:
        ...
        sections.append("Enhanced Visualizations Cont.")
...
def show_home_section():
    ...
    # --- Sample Data Download Options (Cleaned & Uncleaned only) ---
    import os
    ...
    with col1:
        clean_path = os.path.join(os.path.dirname(__file__), "data", "sample_data_10k_cleaned.csv")
    ...
    with col2:
        uncleaned_path = os.path.join(os.path.dirname(__file__), "data", "sample_data_10k_uncleaned.csv")
...
def show_enhanced_visualizations_section(df):
    """Enhanced multi-library visualization showcase (Altair, Seaborn, etc.)"""
    ...
    with viz_categories[0]:
        st.subheader("📈 Interactive Analytics (Altair)")
        if available_libs['altair']:
            # Interactive MPR Distribution
            if 'MPR' in df.columns:
                st.subheader("🎯 Interactive MPR Distribution")
                altair_mpr = create_enhanced_visualization(df, "mpr_distribution", title="Interactive MPR Analysis")
                if altair_mpr:
            ...
            # Interactive Disease Demographics
            if 'Disease_Type' in df.columns:
...
def show_enhanced_visualizations_cont_section(df):
    """Enhanced Visualizations Cont.: Geographic (Folium) and Bokeh Dashboards"""
    st.title("✨ Enhanced Visualizations Cont.")
    st.markdown("### Geographic Analysis (Folium) & Advanced Dashboards (Bokeh)")
    if df is None or df.empty:
        st.warning("Please upload data in the Home section first.")
        return
    available_libs = get_available_libraries()
    viz_tabs = st.tabs([
        "🌍 Geographic Patient Analysis (Folium)",
        "🔧 Advanced Interactive Dashboards (Bokeh)"
    ])
    with viz_tabs[0]:
        ...
        if available_libs['folium'] and 'Region' in df.columns:
            ...
            if map_type == "Patient Distribution":
                ...
                if geo_map:
                    st.components.v1.html(geo_map._repr_html_(), height=600, width=1200, scrolling=False)
                    st.info("""
                    **Map Features:**
                ...
                if hotspot_map:
                    st.components.v1.html(hotspot_map._repr_html_(), height=600, width=1200, scrolling=False)
                    st.info("""
                    **Hotspot Features:**
    ...
    with viz_tabs[1]:
        ...
        if available_libs['bokeh']:
            dashboard_type = st.selectbox(
                ...
                key="bokeh_dashboard_selector_cont"
            )
            if dashboard_type == "Adherence Dashboard" and 'MPR' in df.columns:
                ...
                if bokeh_dashboard:
                    st.components.v1.html(bokeh_dashboard, height=700, width=1200, scrolling=False)
                    st.info("""
                    **Dashboard Features:**
                ...
                if bokeh_timeseries:
                    st.components.v1.html(bokeh_timeseries, height=500, width=1200, scrolling=False)
                    st.info("""
...
def main():
    ...
    elif section == "Enhanced Visualizations Cont.":
        if ENHANCED_VIZ_AVAILABLE:
            show_enhanced_visualizations_cont_section(filtered_data)
        else:
            st.error("Enhanced visualization libraries not available. Please install: altair, bokeh, seaborn, folium")

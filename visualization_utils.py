...
def create_altair_mpr_distribution(df: pd.DataFrame, title: str = "MPR Distribution") -> Optional[alt.Chart]:
    """Create interactive MPR distribution with Altair"""
    ...
    try:
        ...
        # Histogram
        hist = base.mark_bar(opacity=0.7, color='steelblue').encode(
            x=alt.X('MPR:Q', bin=alt.Bin(maxbins=20), title='Medication Possession Ratio (%)'),
        ...
        threshold_text = alt.Chart(pd.DataFrame({'x': [82], 'y': [df.shape[0] * 0.8], 'text': ['80% Threshold']})).mark_text(
            align='left', color='red', fontSize=12, fontWeight='bold'
        ).encode(
            x='x:Q',
            y='y:Q',
            text='text:N'
        ...
        chart = (hist + threshold_line + threshold_text).resolve_scale(
            y='independent'
        ).properties(
            title=title,
            width=600,
        ...
        st.warning(f"Altair chart error: {e}")
        return None
...
def show_visualization_showcase(df: pd.DataFrame):
    ...
    for option in viz_options:
        ...
        if "MPR Distribution" in option and available_libs['altair']:
            ...
            if html:
                st.components.v1.html(html, height=800, width=1200)
